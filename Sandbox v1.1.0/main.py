...
from ui_components import Button, ParticleSelectionButton
...
def main():
    ...
    # Particle categories
    solid_particles = ["Sand", "Dirt", "Stone", "Brick"]
    fluid_particles = ["Water", "Acid", "Oil", "Lava"]
    other_particles = ["Fire"]

    # Load particle images and IDs
    particle_images = {
        "Sand": sand_image, "Dirt": dirt_image, "Stone": stone_image, "Brick": brick_image,
        "Water": water_image, "Acid": acid_image, "Oil": oil_image, "Lava": lava_image, "Fire": fire_image
    }

    particle_ids = {
        "Sand": PARTICLE_IDS["Sand"], "Dirt": PARTICLE_IDS["Dirt"], "Stone": PARTICLE_IDS["Stone"], "Brick": PARTICLE_IDS["Brick"],
        "Water": PARTICLE_IDS["Water"], "Acid": PARTICLE_IDS["Acid"], "Oil": PARTICLE_IDS["Oil"], "Lava": PARTICLE_IDS["Lava"], "Fire": PARTICLE_IDS["Fire"]
    }
    ...
    solid_buttons = create_buttons_for_category(solid_particles, 50)
    fluid_buttons = create_buttons_for_category(fluid_particles, 220)  # Adjust Y start position as needed
    other_buttons = create_buttons_for_category(other_particles, 390)  # Adjust Y start position as needed
    ...
    while running:
        ...
        screen.blit(solid_text, (left_panel_width + center_panel_height + 10, 20))  # Adjust positions as needed
        screen.blit(fluid_text, (left_panel_width + center_panel_height + 10, 190))  # Adjust positions as needed
        screen.blit(other_text, (left_panel_width + center_panel_height + 10, 360))  # Adjust positions as needed
